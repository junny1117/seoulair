...
## 개요
- Flutter 프레임워크를 사용한 서울시의 각 구별로 미세먼지 및 초미세먼지 농도를 실시간으로 확인할 수 있는 웹사이트와 앱
- 미세먼지 농도에 따라 색상으로 상태를 구분.
- 서울시 미세먼지 평균 농도 표출(앱 한정)
- 안드로이드 앱 다운로드 링크 제공(웹사이트 한정)
...
- **서울시 평균 농도**: 앱 한정으로 서울시 전체 평균 미세먼지 농도를 표시
